...
class Gravity_Forms_Themes {
	 ...
	 * @var      string
	 */
	protected $plugin_slug = 'gravity-forms-themes';
	/**
	 * @var      object
	 */
...
// Add shortcodes
		add_shortcode('gravity_forms_themes', array($this, 'render_element'));
		$this->elements = array_merge($this->elements, array(
			'shortcodes'			=>	array(
				'gravity_forms_themes' 			=> '1',
			)
		));
	...
	public function activate_post_types() {
		...
		$args = array(
			'labels' 				=> array(
				'name' 				=> __('Gravity Forms Themes', 'gravity-forms-themes'),
				'singular_name' 	=> __('Theme', 'gravity-forms-themes'),
				'add_new' 			=> __('Add New', 'gravity-forms-themes'),
				'add_new_item' 		=> __('Add New Theme', 'gravity-forms-themes'),
				'edit_item' 		=> __('Edit Theme', 'gravity-forms-themes'),
				'all_items' 		=> __('Themes', 'gravity-forms-themes'),
				'view_item' 		=> __('View Theme', 'gravity-forms-themes'),
				'search_items' 		=> __('Search Themes', 'gravity-forms-themes'),
				'not_found' 		=> __('No Themes defined', 'gravity-forms-themes'),
				'not_found_in_trash'=> __('No Themes in trash', 'gravity-forms-themes'),
				'parent_item_colon' => '',
				'menu_name' 		=> __('Themes', 'gravity-forms-themes')
			),
			'public' 				=>	false,
		...
		register_post_type('gravity_forms_themes', $args);		$this->elements = array_merge($this->elements, array(
			'posttypes'			=>	array(
				'gravity_forms_themes' 			=> 'element',
			)
		));

		add_action('add_meta_boxes', array($this, 'add_metabox'), 5, 4);
		add_action('save_post', array($this, 'save_post_metaboxes'), 1, 2);
		add_filter('manage_gravity_forms_themes_posts_columns', array($this, 'posts_column'),10,2);
		add_action('manage_gravity_forms_themes_posts_custom_column', array($this, 'custom_postcolumn'), 10, 2);
	...
	function updated_messages( $messages ) {
	  ...
	  $messages['gravity_forms_themes'] = array(
	    0 => '', // Unused. Messages start at index 1.
	    1 => sprintf( __('Theme updated.', 'gravity-forms-themes'), esc_url( get_permalink($post_ID) ) ),
	    2 => __('Custom field updated.', 'gravity-forms-themes'),
	    3 => __('Custom field deleted.', 'gravity-forms-themes'),
	    4 => __('Theme updated.', 'gravity-forms-themes'),
	    /* translators: %s: date and time of the revision */
	    5 => isset($_GET['revision']) ? sprintf( __('Theme restored to revision from %s', 'gravity-forms-themes'), wp_post_revision_title( (int) $_GET['revision'], false ) ) : false,
	    6 => sprintf( __('Theme published.', 'gravity-forms-themes'), esc_url( get_permalink($post_ID) ) ),
	    7 => __('Theme saved.', 'gravity-forms-themes'),
	    8 => sprintf( __('Theme submitted.', 'gravity-forms-themes'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
	    9 => sprintf( __('Theme scheduled for: <strong>%1$s</strong>.', 'gravity-forms-themes'),
	      // translators: Publish box date format, see http://php.net/date
	      date_i18n( __( 'M j, Y @ G:i' ), strtotime( $post->post_date ) ), esc_url( get_permalink($post_ID) ) ),
	    10 => sprintf( __('Theme draft updated.', 'gravity-forms-themes'), esc_url( add_query_arg( 'preview', 'true', get_permalink($post_ID) ) ) ),
	  );
	...
	function add_metabox($slug, $post){
		...
		add_meta_box('1321014713', __('Form Settings', 'gravity-forms-themes' ), array($this, 'render_metabox'), 'gravity_forms_themes', 'normal'/* or side */, 'core', array( 'slug' => 'gravity_forms_themes', 'file' => 'form-settings') );
		add_meta_box('315914412', __('Customize', 'gravity-forms-themes' ), array($this, 'render_metabox'), 'gravity_forms_themes', 'normal'/* or side */, 'core', array( 'slug' => 'gravity_forms_themes', 'file' => 'customize') );
	...
	function render_metabox($post, $args){
		...
		echo '<input type="hidden" name="gravity_forms_themes_metabox" id="gravity_forms_themes_metabox" value="'.wp_create_nonce(plugin_basename(__FILE__)).'" />';
		echo '<input type="hidden" name="gravity_forms_themes_metabox_prefix[]" value="'.$args['args']['slug'].'" />';

		include self::get_path( __FILE__ ) . 'configs/' . $post->post_type . '-' . $args['args']['file'] . '.php';
		
		echo "<div class=\"group gravity-forms-themes-row-sorter\" id=\"rowitems\">\r\n";
		// build instance
		$depth = 1;
		...
		for( $i=0; $i<$depth;$i++ ){
			...
			if($i > 0){
				echo '  <div class="button button-primary right gravity-forms-themes-removeRow" style="margin-top:5px;">'.__('Remove', 'gravity-forms-themes').'</div>';
			}
		...
		if(!empty($group['multiple'])){
			echo "<div class=\"toolrow\"><button class=\"button gravity-forms-themes-add-group-row\" type=\"button\" data-rowtemplate=\"group-".$args['id']."-tmpl\">".__('Add Another', 'gravity-forms-themes')."</button></div>\r\n";
		}
		echo "</div>\r\n";
	...
	function posts_column($hd){
		unset($hd['date']);
		$hd['shortcode_slug'] = __('Shortcode', 'gravity-forms-themes');
		$hd['date'] = __('Date', 'gravity-forms-themes');
		return $hd;
	}
	...
	public function enqueue_admin_stylescripts() {
		...
		if($screen->post_type == 'gravity_forms_themes' && $screen->base == 'post'){
	...
	function shortcode_insert_button(){
		global $post;
		if(!empty($post)){
			echo "<a id=\"gravity-forms-themes-shortcodeinsert\" title=\"".__('Gravity Forms Themes Shortcode Builder','gravity-forms-themes')."\" style=\"padding-left: 0.4em;\" class=\"button gravity-forms-themes-editor-button\" href=\"#inst\">\n";
			echo "	<img src=\"". self::get_url( __FILE__ ) . "assets/images/icon.png\" alt=\"".__("Insert Shortcode","gravity-forms-themes")."\" style=\"padding:0 2px 1px;\" /> ".__('Gravity Forms Themes', 'gravity-forms-themes')."\n";
			echo "</a>\n";
		}
	}
	...
	function render_shortcode_panel($shortcode, $type = 1, $template = false){
		...
		if(!empty($template)){
			echo "<script type=\"text/html\" id=\"gravity-forms-themes-".$shortcode."-config-tmpl\">\r\n";
		}
		echo "<input id=\"gravity-forms-themes-shortcodekey\" class=\"configexclude\" type=\"hidden\" value=\"".$shortcode."\">\r\n";
		echo "<input id=\"gravity-forms-themes-shortcodetype\" class=\"configexclude\" type=\"hidden\" value=\"".$type."\">\r\n";
		echo "<input id=\"gravity-forms-themes-default-content\" class=\"configexclude\" type=\"hidden\" value=\" ".__('Your content goes here','gravity-forms-themes')." \">\r\n";
		...
		$groups = array();
		echo "<div class=\"gravity-forms-themes-shortcode-config-nav\">\r\n";
		echo "	<ul>\r\n";
		foreach ($configfiles as $key=>$fieldfile) {
			include $fieldfile;
			$groups[] = $group;
		...
		echo "<div class=\"gravity-forms-themes-shortcode-config-content " . ( count($configfiles) > 1 ? "" : "full" ) . "\">\r\n";
			foreach($groups as $key=>$group){
				echo "<div class=\"group\" " . ( $key === 0 ? "" : "style=\"display:none;\"" ) . " id=\"row".$group['master']."\">\r\n";
				echo "<h3 class=\"gravity-forms-themes-config-header\">".$group['label']."</h3>\r\n";
				...
				if(!empty($group['multiple'])){
					echo "<div class=\"toolrow\"><button class=\"button gravity-forms-themes-add-group-row\" type=\"button\" data-rowtemplate=\"group-".$group['id']."-tmpl\">".__('Add Another','gravity-forms-themes')."</button></div>\r\n";
				}
				echo "</div>\r\n";
			}
		echo "</div>\r\n";
	...
	function shortcode_modal_template(){
		...
		echo "<script type=\"text/html\" id=\"gravity-forms-themes-shortcode-panel-tmpl\">\r\n";
		echo "	<div tabindex=\"0\" id=\"gravity-forms-themes-shortcode-panel\" class=\"hidden\">\r\n";
		echo "		<div class=\"media-modal-backdrop\"></div>\r\n";
		echo "		<div class=\"gravity-forms-themes-modal-modal\">\r\n";
		echo "			<div class=\"gravity-forms-themes-modal-content\">\r\n";
		echo "				<div class=\"gravity-forms-themes-modal-header\">\r\n";
		echo "					<a title=\"Close\" href=\"#\" class=\"media-modal-close\">\r\n";
		echo "						<span class=\"media-modal-icon\"></span>\r\n";
		echo "					</a>\r\n";
		echo "					<h2 style=\"background: url(".self::get_url( '/assets/images/icon.png', __FILE__ ) . ") no-repeat scroll 0px 2px transparent; padding-left: 20px;\">".__('Gravity Forms Themes','gravity-forms-themes')." <small>".__("Shortcode Builder","gravity-forms-themes")."</small></h2>\r\n";
		echo "				</div>\r\n";
		echo "				<div class=\"gravity-forms-themes-modal-body\">\r\n";
		echo "					<span id=\"gravity-forms-themes-categories\">\r\n";
		echo "						<div class=\"gravity-forms-themes-shortcode-name\">".__('Gravity Forms Themes','gravity-forms-themes')."</div><span class=\"gravity-forms-themes-autoload\" data-shortcode=\"gravity_forms_themes\"></span>\r\n";
		echo "					</span>\r\n";
		echo "					<div id=\"gravity-forms-themes-shortcode-config\" class=\"gravity-forms-themes-shortcode-config\">\r\n";
		echo "					</div>\r\n";
		echo "				</div>\r\n";
		echo "				<div class=\"gravity-forms-themes-modal-footer\">\r\n";
		echo "					<button class=\"button button-primary button-large\" id=\"gravity-forms-themes-insert-shortcode\">".__("Insert Shortcode","gravity-forms-themes")."</button>\r\n";
		echo "				</div>\r\n";
		echo "			</div>\r\n";
		echo "		</div>\r\n";
		echo "	</div>\r\n";
	...
	public function get_post_meta($id, $key = null, $single = false){
		...
		if(!empty($key)){
			...
			if(file_exists(self::get_path( __FILE__ ) .'configs/fieldgroups-gravity_forms_themes.php')){
				include self::get_path( __FILE__ ) .'configs/fieldgroups-gravity_forms_themes.php';		
			}else{
				return;
			}
	...
	function save_post_metaboxes($pid, $post){
		...
		if(!isset($_POST['gravity_forms_themes_metabox']) || !isset($_POST['gravity_forms_themes_metabox_prefix'])){return;}


		if(!wp_verify_nonce($_POST['gravity_forms_themes_metabox'], plugin_basename(__FILE__))){
			return $post->ID;
		}
		...
		foreach( $_POST['gravity_forms_themes_metabox_prefix'] as $prefix ){
			if(!isset($_POST[$prefix])){continue;}

			if(isset($_POST['gravity_forms_themes_storage' . $prefix]) && $_POST['gravity_forms_themes_storage' . $prefix] == 'single'){
			...
			//foreach($_POST['gravity_forms_themes'] as $field=>$data){
			/*
			*/
			//}
		}
	...
	public function render_element($atts, $content, $slug, $head = false) {
		...
		if(!empty($head)){
			$instanceID = $this->element_instance_id('gravity_forms_themes'.$slug, 'header');
		}else{
			$instanceID = $this->element_instance_id('gravity_forms_themes'.$slug, 'footer');
		}
